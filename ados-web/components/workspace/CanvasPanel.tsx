...
export function CanvasPanel() {
  const { plan, planStatus, planError, selection, select } = useAdosState()
...
function Thumbnail({ page, active, onClick }: { page: Page; active: boolean; onClick: () => void }) {
  const aspect = page.grid.page_height_mm / page.grid.page_width_mm
  return (
    <button
      onClick={onClick}
      className={`flex-shrink-0 overflow-hidden rounded-[4px] border text-left ${
